...
## Profile files
`profile.example.json` is a legacy version-1 audio profile and exercises automatic migration. `profile.visual.example.json` documents the canonical version-2 audio-plus-visual format and is deliberately an off-face development check, not a wellness protocol. Built-in protocols carry visual channels only where their cited papers actually used light (Micheletti 1998, Swingle 2001, Ochs 2006, Olmstead 2005, Carter & Russell 1993 via Siever's review); every parameter the sources leave unreported stays at a documented neutral default, and visual remains an optional output everywhere.

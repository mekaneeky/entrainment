...
DEFAULT_LOCATIONS = ["Fp1", "Fp2", "F3", "F4", "F7", "F8", "Fz", "C3", "C4", "Cz", "P3", "P4", "Pz", "O1", "O2", "T7", "T8"]
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Build coherence + bandpower Z-score norms and cutoffs from DVS ds005385.")
    parser.add_argument("--dataset", type=Path, default=Path("data/ds005385"), help="Local ds005385 directory")
    parser.add_argument("--glob", type=str, default="sub-*/ses-1/eeg/*_task-EyesClosed_acq-pre_eeg.edf", help="EDF glob")
    parser.add_argument("--locations", type=str, default=",".join(DEFAULT_LOCATIONS), help="Comma-separated locations for bandpower metrics")
    parser.add_argument("--pairs", type=str, default="F3-F4,Fz-Cz,Cz-O1,F3-Cz,F4-Cz", help="Comma-separated coherence pairs")
    parser.add_argument("--max-locations", type=int, default=20, help="Maximum allowed location count")
    parser.add_argument("--min-age-bin-n", type=int, default=10, help="Minimum samples required to emit per-age-bin stats")
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("backend/clinicalq_backend/data/coherence_norms_dvs_608.json"),
    ...
    if not edf_paths:
        raise RuntimeError(f"No EDF files matched {args.glob} under {dataset_dir}.")
    ...
    needed = sorted({loc for pair in pairs for loc in pair} | set(locations))
    keys = [f"COH:{a}-{b}:{band}" for a, b in pairs for band in BANDS]
    keys.extend([f"BP:{loc}:{band}" for loc in locations for band in BANDS])
    ...
    for idx, edf_path in enumerate(edf_paths, start=1):
        ...
        pick_names = [ch_map[loc.upper()] for loc in needed]
    ...
    out = {
        ...
        "task": "EyesClosed",
        "acquisition": "pre",
        "notes": "DVS ds005385 coherence + bandpower norms. Cutoffs are mean +/- 2 SD.",
    ...
    print(
        f"Wrote {args.output.resolve()} (n_files={used_files}, n_subjects={len(used_subjects)}, locations={len(locations)})",
        flush=True,
    )
    return 0
